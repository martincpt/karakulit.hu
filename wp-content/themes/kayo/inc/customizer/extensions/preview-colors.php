<?php
/**
 * Kayo customizer colors preview functions
 *
 * @package WordPress
 * @subpackage Kayo
 * @version 1.5.1
...
/**
 * Registers color schemes for Kayo.
 *
 * Can be filtered with {@see 'kayo_color_schemes'}.
 *
 ...
 */
function kayo_get_color_schemes() {
	/**
	 * Filter the color schemes registered for use with Kayo.
	 *
 ...
 */
function kayo_get_color_scheme_css( $colors ) {
	...
	extract( $colors ); // phpcs:ignore

	$output = '';

	$border_color           = sprintf( 'rgba( %s, 0.08)', kayo_hex_to_rgb( $strong_text_color ) );
	...
	***********************/
	...
	/**
	 * Filters color scheme CSS output
	 * 
	 * @since Kayo 1.0.0
	 */
	return apply_filters( 'kayo_color_scheme_output', $output, $colors );
}
 ...
 * @return array $colors
 */
function kayo_get_template_colors() {
 ...
 * Customizer preview.
 */
function kayo_color_scheme_css_template() {

	$colors = kayo_get_template_colors();
	?>
	<script type="text/html" id="tmpl-kayo-color-scheme">
		<?php echo kayo_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'kayo_color_scheme_css_template' );
