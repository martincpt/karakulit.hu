...
;( function( $ ) {
	 ...
	 * Background
	 */
	var colorSchemeStyle = $( '#kayo-color-scheme-css' ),
		fontsStyle = $( '#kayo-fonts-css' ),
		layoutStyle = $( '#kayo-layout-css' ),
		api = wp.customize,
		backgrounds = {
	...
	if ( ! colorSchemeStyle.length ) {
		colorSchemeStyle = $( 'head' ).append( '<style type="text/css" id="kayo-color-scheme-css" />' )
			.find( '#kayo-color-scheme-css' );
	}

	// Fonts style tag
	if ( ! fontsStyle.length ) {
		fontsStyle = $( 'head' ).append( '<style type="text/css" id="kayo-fonts-css" />' )
			.find( '#kayo-fonts-css' );
	}

	// Layout style tag
	if ( ! layoutStyle.length ) {
		layoutStyle = $( 'head' ).append( '<style type="text/css" id="kayo-layout-css" />' )
			.find( '#kayo-layout-css' );
	}
	...
	api( 'menu_width', function( value ) {
		value.bind( function( to ) {
			...
			KayoUi.centeredLogoOffset();
			KayoUi.subMenuDirection();
		} );
	} );
